...
filename <- "wallhaven爬虫"
if(! dir.exists(filename)){
  dir.create(filename)
}


url <- 'https://wallhaven.cc/search?categories=010&purity=100&resolutions=1920x1080&topRange=1y&sorting=toplist&order=desc&page=1'
#图片详情页网址
...
for(i in 1:length(picture)){
  ...
  download_url <- read_html(picture[i]) %>%
      ...
      html_attr("src") %>% print()
  download(download_url,paste(filename,"/",i,str_sub(download_url,-4,-1),sep = ""), mode = "wb")
  Sys.sleep(2)
} 
